...
BEGIN
  ...
  FOR idx IN 1..1000 LOOP
    ...
    DELETE FROM topic WHERE post = post_handle;

    --DELETE FROM post WHERE handle = post_handle;
    DELETE FROM post WHERE author = author_handle;

    --DELETE FROM author WHERE handle = author_handle;
    DELETE FROM author WHERE usr = usr_handle;

    --DELETE from contact WHERE handle = contact_handle;
    DELETE from contact WHERE usr = usr_handle;
