...
module.exports = {
  ...
  get: (obj, property, proxy) => {
    ...
    if (!val) {
      ...
      if (handle) {
        ...
        let val = obj.constructor.getValue(element, property, handle);
  ...
  set: (obj, property, val, proxy) => {
    ...
    if (property == 'handle') {
      let values = obj.constructor.getAllValues(element, val);
    ...
    if (handle) {
      obj.constructor.setValue(element, property, handle, val);
